...
print(transform_user_details("user_details.csv")[1:])  # displays all info apart from the headers
...
def create_new_user_data_csv(old_user_data_file="user_details.csv", new_file_name='new_user_data.csv'):
    ...
    new_file = open(new_file_name, 'w')  # open file first, then create an object that we use to write to the file

    with new_file:
        csv_writer = csv.writer(new_file)  # csv writer object, passing it to the file we wish to write to, then specifying what we want to write
